...
import { Species, Person, Film, Homeworld } from "~/sw-types/general-interface";
...
const fetchAdditionalDetails = async (species: Species) => {
  const homeworldPromise = species.homeworld
    ? fetchHomeworld(species.homeworld)
    : Promise.resolve();
  const peoplePromise = species.people
    ? fetchPeople(species.people)
    : Promise.resolve();
  const filmsPromise = species.films
